...
def test_create_sync_creates_issue_and_writes_back(monkeypatch, tmp_path: Path):
    ...
    resp = client.post("/api/zammad/create-sync", json=payload, headers=headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["success"] is True
...
def test_create_sync_skips_if_erp_issue_exists(monkeypatch, tmp_path: Path):
    ...
    headers = {"Authorization": "Bearer test-token"}
    payload = {
        "zammad_ticket_id": 1000,
        "zammad_ticket_number": "68000",
        "customer_name": "Ivan",
        "erp_issue_ref": "ISS-2026-00999",
    }
    resp = client.post("/api/zammad/create-sync", json=payload, headers=headers)
    assert resp.status_code == 200
    body = resp.json()
    ...
    assert body["erpnext_issue"] == "ISS-2026-00999"
